...
export default function SingleVehicleDetails({ params }: { params: { id: string } }) {
    ...
    function requestToCheckOutHandler() {
        if (isUserAuthenticated) {
            if (profileVerifiedStatus()) {
                ...
                // const url = `/checkout/id=${vehicleId}?startDate=${formattedStartDate}&endDate=${formattedEndDate}`;

                window.location.href = `/checkout/${vehicleId}?${pickupDate}?${dropDate}?${pickupTime}?${dropTime}?${vehicleDetails.price_per_hr}`;

                console.log({ formattedStartDate, formattedEndDate });
            } else {
                alert('Your profile is not verified, please verify and checkoout');
            }
        } else {
            alert('You are not logged in , Please login and try to checkout');
        }
    }
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 mt-3 md:mt-6'>
                        ...
                        <div className='mt-4 lg:row-span-3 lg:mt-0'>
                            ...
                            <div className='mt-10'>
                                ...
                                <Button type='submit' className='mt-10 flex w-full' disabled={!!error} onClick={requestToCheckOutHandler}>
                                    Request to checkout
                                </Button>
                            </div>
                        </div>
